...
//多层map也可解码但是decode定义的第二个参数要和encode输入的类型保持一致
func decodeMap(typ reflect.Type) (decoder, error) {
	decoder := func(s *Stream, val reflect.Value) (err error) {
		var temp string //暂时将解码结果放在temp
		err = decodeString(s, reflect.ValueOf(&temp).Elem())
		if err != nil {
			return err
		}
		//fmt.Println(temp)
		t := reflect.New(typ).Interface()  //new新map防止json反序列化时空值
		json.Unmarshal([]byte(temp), t)    //string转换为map
		val.Set(reflect.ValueOf(t).Elem()) //将解码结果返回给val
		return nil
	}
	return decoder, nil
}
