...
func makeStructWriter(typ reflect.Type) (writer, error) {//struct的成员要大写
...
func makeMapWriter() (writer, error) {
	writer := func(val reflect.Value, w *encbuf) error {
		s := val.Interface() //获取val的值
		mjson, _ := json.Marshal(s)
		maptostring := string(mjson) //转化为string
		err := writeString(reflect.ValueOf(maptostring), w)
		if err != nil {
			return err
		}
		return nil
