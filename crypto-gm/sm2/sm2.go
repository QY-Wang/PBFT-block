...
type PublicKey struct {
	elliptic.Curve `json:"elliptic.Curve"`
	X              *big.Int `json:"X"`
	Y              *big.Int `json:"Y"`
}
...
//}
func SignDigitToSignData(v []byte, r, s *big.Int) ([]byte, error) {
	return asn1.Marshal(sm2Signature{v, r, s})
}
...
//}
func SignDataToSignDigit(sign []byte) ([]byte, *big.Int, *big.Int, error) {
	var sm2Sign sm2Signature
	...
	return sm2Sign.V, sm2Sign.R, sm2Sign.S, nil
}
...
func (priv *PrivateKey) Sign(rand io.Reader, msg []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	return asn1.Marshal(sm2Signature{nil, r, s})
}
...
func (prv *PrivateKey) GenerateShared(pub *PublicKey, skLen, macLen int) (sk []byte, err error) {
	...
	if pub.X == nil {
		return nil, errors.New("pubkey err")
	}
	//fmt.Println(pub.X, pub.Y, prv.D.Bytes())
	...
	skBytes := x.Bytes()
	//fmt.Println("len(sk),len(skBytes)",len(sk),len(skBytes))
