...
import (
	...
	"github.com/DWBC-ConPeer/metrics"
...
// meteredMsgReadWriter 是 p2p.MsgReadWriter 的包装器，能够根据数据流内容累积上述定义的指标。
type meteredMsgReadWriter struct {
	p2p.MsgReadWriter     // Wrapped message stream to meter
	version           int // Protocol version to select correct meters 选择正确仪表的协议版本
}
...
func (rw *meteredMsgReadWriter) ReadMsg() (p2p.Msg, error) {
	...
	//case msg.Code == RequsetMsg: //add the pbft message
	case msg.Code == TestSendMsg: //add the pbft message //FIXME

	case msg.Code == TestReciveMsg: //add the pbft message //FIXME
...
func (rw *meteredMsgReadWriter) WriteMsg(msg p2p.Msg) error {
	glog.V(logger.Info).Infoln(" is that WriteMsg metrios ?")
	// Account for the data traffic
	...
	return rw.MsgReadWriter.WriteMsg(msg) // 看不懂?
}
