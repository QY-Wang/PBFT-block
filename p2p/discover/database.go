...
func (db *nodeDB) GetNode(id NodeID) *Node {
	return db.node(id)
}

// updateNode inserts - potentially overwriting - a node into the peer database. updateNode 将一个节点插入（可能覆盖）到对等数据库中。
...
/*
* 通过更新 nodetype 更新node
 */
func (db *nodeDB) UpdateNodeByType(node *Node) error {
	blob, err := rlp.EncodeToBytes(node)
	if err != nil {
		return err
	}
...
// findFails retrieves the number of findnode failures since bonding. findFails 检索自绑定以来 findnode 失败的次数。
